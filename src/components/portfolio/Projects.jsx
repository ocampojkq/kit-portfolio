import useProjects from "../../hooks/useProjects";

export default function Projects({ darkMode }) {
  const projects = useProjects();

  return (
    <section
      id="projects"
      className={`py-16 px-6 ${darkMode ? "bg-gray-800" : "bg-white"}`}
    >
      <div className="max-w-6xl mx-auto">
        <h2
          className={`text-3xl font-bold mb-2 text-center ${darkMode ? "text-white" : "text-gray-900"}`}
        >
          Projects
        </h2>
        <p
          className={`text-center mb-10 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
        >
          Live projects I've built
        </p>

        {!projects || projects.length === 0 ? (
          <div className="text-center text-gray-400 py-10">
            Loading projects...
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project) => (
              <div
                key={project.title}
                className={`rounded-xl p-6 border hover:border-blue-500 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 flex flex-col ${
                  darkMode
                    ? "bg-gray-900 border-gray-700"
                    : "bg-white border-slate-200 shadow-md"
                }`}
              >
                {project.featured && (
                  <span className="bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full font-semibold w-fit mb-3">
                    Featured
                  </span>
                )}
                <h3
                  className={`font-bold text-lg mb-2 ${darkMode ? "text-white" : "text-gray-800"}`}
                >
                  {project.title}
                </h3>
                <p
                  className={`text-sm mb-4 flex-1 ${darkMode ? "text-gray-400" : "text-gray-600"}`}
                >
                  {project.description}
                </p>

                {/* Tech Badges Container with String Safety Check */}
                <div className="flex flex-wrap gap-1 mb-4">
                  {(typeof project.tech === "string"
                    ? project.tech.split(",")
                    : []
                  ).map((t) => (
                    <span
                      key={t}
                      className={`text-xs px-2 py-0.5 rounded-full ${
                        darkMode
                          ? "bg-gray-700 text-gray-300"
                          : "bg-blue-50 text-blue-700"
                      }`}
                    >
                      {t.trim()}
                    </span>
                  ))}
                </div>

                <div className="flex gap-3">
                  {/* FIXED: Restored missing <a tag definition for Live Demo */}
                  {project.liveUrl && (
                    <a
                      href={project.liveUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="flex-1 text-center bg-blue-600 text-white px-3 py-2 rounded-lg text-sm hover:bg-blue-700 transition font-medium"
                    >
                      Live Demo
                    </a>
                  )}

                  {/* FIXED: Restored missing <a tag definition for GitHub */}
                  {project.githubUrl && (
                    <a
                      href={project.githubUrl}
                      target="_blank"
                      rel="noreferrer"
                      className={`flex-1 text-center px-3 py-2 rounded-lg text-sm border transition font-medium ${
                        darkMode
                          ? "border-gray-600 text-gray-300 hover:bg-gray-700"
                          : "border-gray-300 text-gray-700 hover:bg-gray-50"
                      }`}
                    >
                      GitHub
                    </a>
                  )}
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </section>
  );
}
